...
| An alert's area is worked out | `app/mapgeometry.go:resolveAlertAreas` — an alert that carries its own polygon keeps it; **four in five carry none** and name forecast zones instead, and those are resolved through `domains/weather/nws/zones/zones.go:Zones`.  It lives in `app/` because the store is a domain and whatever draws is under `platform/`, and neither `modes/` nor `platform/` may import a domain |
| A zone's outline is fetched | `domains/weather/nws/zones/zones.go:Zone` — once per id, kept whole with its name.  The freshness the service declares, the disk tier and the conditional refresh are the client's (`platform/httpx/cache.go:serverTTL`), not this package's.  The watched places' own zones are taken at start-up by `domains/weather/nws/zones/zones.go:Seed` |
| A geometry is read from the wire | `platform/geo/geojson.go:ReadGeometry` — token-streamed and **never recursed**, because decoding into an interface recurses once per array level with no ceiling (the finding `domains/globalfeed/nws.go:geoPoint` was written against).  Depth is a counter; positions are found at the depth the declared type says, never guessed |
| Rings are kept grouped into the areas they belong to | `platform/geo/geojson.go:layoutFor` — each geometry type declares at which bracket depth a position, a ring and an **area** close.  It is why a county and its lake are one area of two rings while a coast is one area per island, and the grouping cannot be recovered afterwards: an island and a hole are the same list of positions.  What draws reads an area's first ring as its outline and **every ring after it as a hole** |
| An alert keeps the shape it arrived with | `domains/weather/nws/alerts.go:alertFrom` — the response models `geometry` now, so it is no longer discarded at decode.  `domains/weather/nws/alerts.go:mapAlert` matches on the FULL zone list while only the stored copy is bounded (R3-A-01) |
| An earthquake keeps where it happened | `domains/seismic/usgs/usgs.go:stateFor` keeps `Lat` and `Lon` beside the distance and compass word it computes from them — a distance and a bearing cannot be turned back into an epicentre |
