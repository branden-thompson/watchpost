...
| The radio tunes | `app/radio.go:Tune` → `domains/radio/stream/resolve.go:ResolveWithStatus` → `app/radio.go:tuneList` → the engine `domains/radio/player/engine.go:Start`; with nothing live carrying the location, `app/radio.go:needsRead` reports the fact and the SCHEDULE decides — `platform/lineup/rotation.go:onNeedsRead` makes the card, `app/executors.go:speak` takes the air, and `app/radio.go:readReport` plays it |
