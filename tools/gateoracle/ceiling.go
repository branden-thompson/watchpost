...
func ceiling(t Reporter, did string) {
	...
	t.Logf("%s. The oracle enforces against DRIFT and not EVASION. NOT judged: a script sourced or inlined "+
		...
		"discard inside a script, a binary `go build` writes without `-o`, a discarded `-go build` whose binary "+
		"is absent, a third-party `go run` tool's control, the ABSENCE of a tool the OS ships (%v), which of "+
		"two PARALLEL invocations takes #1, the per-key count under verify, and any command not in %v. Git "+
		"metadata is judged as CI has it: HEAD detached at the commit, no tags, the working tree committed.",
		did, osShipped(), tools())
}
