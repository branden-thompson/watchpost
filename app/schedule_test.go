...
func scheduleUnderTest(t *testing.T, ctx context.Context) (*schedule, *atomic.Int64) {
	...
	s := startSchedule(ctx, nar, nil, func() render.Clock { return render.Clock12 }, nil, nil, nil, tick, nil, nil)
	if s == nil {
...
func TestNoArbiterMeansNoSchedule(t *testing.T) {
	tick := &tickerDeck{muted: &atomic.Bool{}, seen: loadSeen(t.TempDir(), time.Hour), alerts: newAlertStore()}
	if s := startSchedule(context.Background(), nil, nil, func() render.Clock { return render.Clock12 }, nil, nil, nil, tick, nil, nil); s != nil {
	...
	if s := startSchedule(context.Background(), nar, nil, func() render.Clock { return render.Clock12 }, nil, nil, nil, nil, nil, nil); s != nil {
...
func TestTheProducersReachTheSchedule(t *testing.T) {
	...
	tick := &tickerDeck{muted: &atomic.Bool{}, seen: loadSeen(t.TempDir(), time.Hour), alerts: newAlertStore()}
	deck := &radioDeck{}
	s := startSchedule(ctx, nar, nil, func() render.Clock { return render.Clock12 }, deck, nil, nil, tick, nil, nil)
	if s == nil {
...
func TestTheScheduleTopsTheLineUpOffToTheConsolesWindow(t *testing.T) {
	...
	s := startSchedule(ctx, nar, nil, func() render.Clock { return render.Clock12 }, nil,
		func() []snapshot.LocationRef { return refs }, func() []snapshot.LocationRef { return refs }, tick, publish, nil)
	if s == nil {
