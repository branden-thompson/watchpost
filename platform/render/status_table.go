...
func (o Opts) StatusTable(cols []StatusColumn, rows []StatusRow, inner int, headerTone string) []string {
	...
	// splitCells, not TruncateCells: the lines are STYLED, and TruncateCells
	// counts an escape sequence's bytes as content and will cut through the
	// middle of one — which the terminal then prints as text.
