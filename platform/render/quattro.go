...
func quattroForegrounds(p quattroPalette) map[Token]string {
	return map[Token]string{
		...
		// AND THE CONSOLE'S WORD IN THE PALETTE'S OWN ORANGE (D-133). Every
		// Quattro palette declares one, so this needs no per-theme decision —
		// the same reason the blue above is derived rather than listed.
		TitleEditionBroadcaster: boldFg(p.orange),
