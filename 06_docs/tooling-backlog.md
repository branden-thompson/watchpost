...
| 5 | `P10-09` pointer discipline | `depguard` on `unsafe`/`reflect` in `.golangci.yml`, plus AST for `**T` | config + AST | ~0 | ~15 lines | "Cheapest real P10 win" |
| 6 | `SEC-01` fixed-argv | AST: `exec.Command(Context)` with a non-constant program argument, or `sh`/`bash`/`cmd`/`powershell` plus `-c` | AST | ~0 | ~20 lines in `tools/authoring` | Pure ratchet on a catastrophic class — all four live sites are correct today |
| 7 | `AP-STALE-01` proxy | every `go test` in a gate recipe carries `-count=1`, so no gate is answerable from cache | regex (`makeLines` harness exists) | 0 | ~15 lines | — |
