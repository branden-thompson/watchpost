...
### Session 117 (publish: main + v0.9.0)

| # | Item | Disposition |
|---|---|---|
| 117.1 | REVIEW approved; SQUASH approved; publish under the branden-thompson account | **Done**: `main` = one orphan commit from the feature tree (247 files, no harness files), pushed over the personal SSH alias; repo created public with the personal `gh` account (verified `gh api user` before every outward step). |
| 117.2 | The release job failed three times on CI-only conditions the Mac never hit | Fixed forward on `main` (never a rebase; the tag moved only because no release had published): (1) a darwin-only expectation in `TestVoiceChipLabelIsTheChooserLabel` — Linux reads an empty chip before Piper installs by design; (2) `lint-watermark` ran `git log main..HEAD` on a detached tag checkout → exit 128 under `set -e`; (3) the 10 ms type-ahead budget under `-race` on a shared runner (13.9 ms) — 60 ms under the race detector via a build-tagged constant. Fourth run: verify, matrix, installer smoke test, publish — all green. |
| 117.3 | Arch / CachyOS (HUM LEAD) | Supported as any glibc Linux; the player loads `libasound.so.2` at runtime (PipeWire via `pipewire-alsa`). Protocol §0 checks `pacman -Q glibc alsa-lib pipewire-alsa`. |

### Session 118 (Arch/CachyOS validation — Linux voice profiles)

| # | Item | Disposition |
|---|---|---|
| 118.1 | HUM LEAD on CachyOS: install OK, Synth works, no Piper install prompt noticed, `[V]` shows no profiles — after the first play, only "Lessac" | **Root cause**: on Linux the chooser listed only what `FindPiper` found under the cache dir — nothing before the first tune-in, one voice after; the progress line (`installing the Piper voice… NN%`) is real but short on a fast connection. **Fixed** (v0.9.1): a curated Piper catalogue — Lessac, Amy, Ryan, Joe (en_US), Alan, Alba (en_GB), medium 22.05 kHz, each ~63 MB, pinned SHA-256 + size from rhasspy/piper-voices v1.0.0 (hashed 2026-08-25) — is what `[V]` lists on Linux/Windows, installed or not. A pick or a preview of a voice not yet on disk downloads it (`EnsureVoice`: binary once, voice files per voice; one install at a time) with `installing Amy voice… NN% (MB)` in the player; the chip reads the installed voice (empty before any install); the sign-off says the chosen name. Pinned: catalogue integrity (six distinct checksums), lookup by name/key, `FindPiperVoice`/`InstalledVoices`/`FindPiper` over a fake tree, chooser + spec resolution (Linux-gated test; CI runs it). |

### Session 119 (Voice chooser: the wait, explained)
