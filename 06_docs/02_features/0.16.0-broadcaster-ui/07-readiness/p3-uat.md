...
**SUPERSEDED 2026-09-17.** The console ships live; the P3(a4) rows of `gates.md` are its gate record. `modes/tty/broadcaster_uat_test.go` is ten rendering tests for one truncation defect, not this checklist. **HUM LEAD UAT, 2026-09-18, on a local build after the REVIEW remediations (ruling 11): the Observer pass — good; the Broadcaster pass — good; case 10, the thirty-minute rotation, signed off separately the same day ("30 min works fine on my testing").** This document records the dark state it was written for.
