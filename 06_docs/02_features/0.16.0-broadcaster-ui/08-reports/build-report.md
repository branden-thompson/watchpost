...
| **FR-4.3** | The cut-over is operator-initiated, and the duck's behaviour on it is decided | TRACED (no ID) — the automatic half | `TestTheDirectorsTuneLeavesTheDuckAlone` (`app/schedule_test.go`) holds the AUTOMATIC tune; the PRESSED cut-back under a reading hazard is untested at the Director and emits `Restore` there (business reviewer, Finding 4) — audio holds through the arbiter's re-dip today, and the test is owed with F-158. `inject_scenarios_test.go` cites an earlier FR-4.3 |
...
| **FR-8.4** | A tiny radius must still receive county and zone products.  The mechanism already | TRACED (no ID) | `TestAThreeMileStationStillReceivesItsCountyWarning` (`platform/lineup/fence_test.go`) — the fence at THREE miles admits the county product by the tracked tie and refuses a point five miles out; `TestCountyUGCFromResolvedPoint` (the county UGC from the point). `TestAZoneOnlyAlertTheAppIsTrackingSurvivesTheRadius` is the Observer's scope at 100 miles, not this requirement (business reviewer) |
...
| **FR-9.4** | The tower's storage boundary is stated to the operator.  The tower is a real person's | TRACED | 2 roster row(s), `app/dump_test.go`, `modes/tty/setup_station_test.go`; `TestTheRadioDiagnosticNamesPlacesNotCoordinates` (`app/radio_debuglog_test.go`) — the radio diagnostic too (ruling 10) |
