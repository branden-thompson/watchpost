...
| FR-5.6 *(P1)* | Radar is built against tagged go-tuiMaps releases only: release candidates (`v0.2.0-rc.N`) during BUILD, as the work packages P1-b needs land; **0.18.0 ships only against the final `v0.2.0`** | D-11, D-51 | `go.mod` requires a tagged version, never a local replace; at SHIP, the final tag |
...
| FR-7.4 *(P1)* | **A text description ships in this release** (macro phase 1): where the picture cannot be drawn or read — `--ascii`, a font without braille, a screen reader — the window renders `Describe` data as an aggregate of the location's hazard and forecast facts, as text; speaking it waits for the voice pass (D-52, F-181). It is also the fallback under every failure mode FR-3.4 contemplates | D-26, D-52; red team A-1/A-5; go-tuiMaps D-122 | **Acceptance: M1b** — a listener answers the covers / stops short / lies to one side question from the description alone. Scored by M1b and never by M1, whose own rule hides the text and demands the picture answer |
...
| FR-9.6 *(P1)* | **The loop's playback speed is a Setting**: 200 ms to 1000 ms a frame, 500 ms by default (go-tuiMaps D-76). The choices below 400 ms lift the library's flash ceiling, and the row says so beside them; reduce motion still forces playback off | D-59 | Settings round trip; the step reaches the map; the fast choices carry the ceiling's note |
...
- **M6 — Loop smoothness** *(secondary, D-18; instrument D-53)*. The radar loop holds its frame rate
  without stalling the rest of the Observer — measured as the intervals between delivered map ticks and
  the key-to-response latency, with the Observer live; thresholds measured in BUILD and ruled; recorded
  at SHIP, never asserted in the gate. The one way D-10 could fail that M1–M5 would not catch.
