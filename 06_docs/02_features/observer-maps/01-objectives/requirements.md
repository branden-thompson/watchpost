...
| FR-3.7 *(P1)* | Every drawn source is credited at every width; `SourceCredit()`'s HTML is never printed raw | R-3.3; W1-B | Golden at the narrowest width |
...
| FR-3.10 *(P1)* | **Until HR-8 lands, the map tile cache's retention is unenforceable and the record says so**: the library's `CacheRoot(dir, capBytes)` takes bytes only — no age, no purge — so FR-3.9's map half is a byte cap plus a watchpost-side delete of the directory, and nothing finer | R2 InfoSec F-4 | Test: the clear path removes the tile directory |
...
| FR-5.7 *(P1)* | A radar fetch carries an explicit body cap (≤ 1 MiB against measured 7.7–19.5 KB frames), **enforced inside the HTTP client as it reads** — `platform/httpx` caps bodies at a 32 MB package constant and writes to the cache before returning, so a caller-side check fires after the bytes are already held. An over-cap body is refused and **never cached** | D-31; red team F3; R2 InfoSec F-3 | Test asserting the cache is **empty** after an over-cap response, not merely that the caller errored |
| FR-5.8 *(P1)* | **Map** motion is a Settings row wired to the library's `ReduceMotion` (a boolean in v0.1.0), with a still form that keeps the newest frame and its age. Switching it off never costs the listener the radar data. The row is labelled for the map, because the Observer animates elsewhere at 300 ms and 50 ms (`modes/tty/dashboard.go:835,897`) and this switch does not reach those — an app-wide motion setting is F-180 | D-27; red team A-7, R2 A11y F-5/F-6 | Settings test; a still-form golden |
| FR-5.9 *(P1)* | A **maximum loop frame rate**, stated as a number. **NO INSTRUMENT YET** — the number is set at PLAN: go-tuiMaps' NFR-21 is a *flash* ceiling (2.5 per second), not a loop rate, and loops do not exist until v0.2.0, so no source for this figure exists today. **HR-9** asks v0.2.0 for loop-rate control and for what `ReduceMotion` means over frames | R2 A11y F-5, R2 PM D1 | NO INSTRUMENT YET |
...
| FR-7.3 *(P1)* | Coordinates are never spoken | R-7.3, MG-R4 | Existing speech guard extended |
