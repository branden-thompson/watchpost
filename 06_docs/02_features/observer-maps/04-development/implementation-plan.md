...
| W5.2 | **One overlay per alert**, one feature per area; severity to role; times and sender carried; **the severity word in the label from P1-a** (FR-4.2, D-55) | `app/mapfeed.go` | — | Round trip against the library's own reading of rings; each label carries its severity word |
