...
| W1.11 | Settings rows: default scale, layers, alert scope (FR-9.1, FR-2.3, FR-4.3) | same | — | Settings round trip per row *As built (batch 9): Default scale, Nearby and Layers on the Maps tab, saved with the group; alert scope is W5.3's, batch 10.* |
