...
| 2.10 | The tone path: the benchmark on the 0.14.0 tree ≤ row 1.1 + 50 % (the pin); the live budget — `WATCHPOST_DEBUG_RADIO=1` (0.15.0: a NAME, not a path — the log lands under the cache root), a takeover, `breaking:` → `tone:` ≤ 250 ms with the root installed **and** on a fresh HOME with nothing installed (`perf-protocol.md` §1 item 2) | P4 (release build) | |
